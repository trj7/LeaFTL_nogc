...
class Ftl(ftlbuilder.FtlBuilder):
        ...
        for frame in self.metadata.mapping_table.frames.values():
            for run in frame.runs:
                ...
                for seg in run:
                    ...
                    if seg.x1 == seg.x2: 
                        length = len(seg._points)
                        single_point_count += length
    ...
    def clean(self, forced=False, merge=True):
        return
        print('written_bytes:{}'.format(self.written_bytes))
...
class BlockValidityCounter(object):
    ...
    def __init__(self, conf):
        ...
        print('n_blocks_per_channel:{}'.format(self.conf.n_blocks_per_channel))
...
class FrameLogPLR:
    ...
    def lookup(self, lpn, first=True):
        ...
        if frame_no in self.frames:
            ...
            # print("lpn:{};frame:{};result:{};frame:{};ppn:{}".format(lpn,frame_no,results,frame, self.metadata.reference_mapping_table.get((lpn))))
    ...
    def should_flush(self):
        if self.memory > self.max_size:
            ...
            # print("Mapping_table_size:{}".format(self.memory))
            return False
...
def split_ext(extent):
    ...
    exts = []
    for lpn in extent.lpn_iter():
        cur_ext = Extent(lpn_start=lpn, lpn_count=1,timeStamp = extent.timeStamp)
        exts.append(cur_ext)
